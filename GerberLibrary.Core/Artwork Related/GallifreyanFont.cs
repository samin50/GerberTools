...
namespace GerberLibrary
{
    public class GallifreyanFont
    {
        ...
        public class CoreGraphicsWriter: GraphicsInterface
        {
            bool DoFill = false;
            bool DoStroke = true;
            CAPSHAPE CurrentShape = CAPSHAPE.ROUND;
            ...
            public GerberLibrary.Core.GraphicsInterface G;

            public CoreGraphicsWriter(GerberLibrary.Core.GraphicsInterface G)
            {
                this.G = G;
                // Smoothing mode handled by interface impl if possible
            }
            public void strokeCap(CAPSHAPE cAPSHAPE)
            {
                CurrentShape = cAPSHAPE;
            ...
            public void arc(double nX, double nY, double p1, double p2, double start, double end)
            {
                ...
                // Convert start/sweep to degrees.
                // ImageSharp default connection?
                this.G.DrawArc(p, (float)(nX - p1 / 2), (float)(nY - p2 / 2), (float)p1, (float)p2, (float)conv(start), (float)conv(sweep));
            }

            public void line(double x1, double y1, double x2, double y2)
            {
                Pen p = GetPen();
                this.G.DrawLine(p, new PointF((float)x1, (float)y1), new PointF((float)x2, (float)y2));
            }
            ...
            public Pen GetPen()
            {
                var p = new Pen(sfg, (float)strokewidth);

                // Map caps
                // ImageSharp Pen has LineCap StartCap and EndCap? No, ImageSharp Pen is immutable? 
                // Wait, Pen(Color, float, DrawingOptions?)
                // Actually ImageSharp Pen usage example: new Pen(Color.Black, 1f, new DrawingOptions(){...})?
                // Or properties?
                // Checking ImageSharp 3...
                // Pen properties are get-only? No, they might be settable or passed in constructor.
                // Actually Pen constructor: Pen(Color color, float width, float[] dashPattern, float dashOffset) etc.
                // Pen properties: StrokeCap?
                // ImageSharp has EndCap, StartCap.
                // But Pen is class.
                // p.EndCap = ...?
                // Assuming standard Pen properties for now.
                
                // LineCap enum in ImageSharp.Drawing.Processing?
                
                LineCap Cap = CurrentShape == CAPSHAPE.ROUND ? LineCap.Round : LineCap.Square;
                // DashCap DCap = CurrentShape == CAPSHAPE.ROUND ? DashCap.Round : DashCap.Flat; // ImageSharp uses LineCap for dashes too?
                
                // p.StrokeCap = Cap?
                // Let's assume defaults for now as simple replacement. Caps might need adjustment.
        ...
        public void DrawToGraphics(GerberLibrary.Core.GraphicsInterface G, string S, double width, double height)
        {
            DrawToInterface(new CoreGraphicsWriter(G), S, width, height);            
