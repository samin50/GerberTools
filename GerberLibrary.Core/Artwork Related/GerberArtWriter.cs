    ...
    namespace ArtWork
    {
        ...
        public class Functions
        {
            ...
            private static void ArtGenerator_ProtoEdge(ProgressLog log,   string outline, string soldermask, string target, string silk)
            {
                ...
                Image<Rgba32> B = Image.Load<Rgba32>("protoedge.png");
                double Res = 200.0 / 25.4;
                B.Mutate(x => x.Flip(FlipMode.Vertical));
            ...
            public static ParsedGerber BuildMazeFromBitmap(string filename)
            {
                ...
                Gerb.Shapes.Clear();
                Image<Rgba32> B = Image.Load<Rgba32>(filename);
                ...
                // Helper for brightness (HSL lightness)
                float GetBrightness(Rgba32 c) 
                {
                    float r = c.R / 255f;
                    float g = c.G / 255f;
                    float b = c.B / 255f;
            ...
            private static void ArtGenerator_ReactDiffuse(ProgressLog log, string outline, string soldermask, string target, string silk)
            {
                ...
                Image<Rgba32> B = new Image<Rgba32>(iW, iH);
                Image<Rgba32> B2 = new Image<Rgba32>(iW, iH);
                Image<Rgba32> B3 = new Image<Rgba32>(iW, iH);
                var G2 = new ImageSharpGraphicsInterface(B3);
                ...
                var G = new ImageSharpGraphicsInterface(B);
                G.Clear(Color.Black);
                ...
                foreach (var a in CombinedOutline)
                {
                    ...
                    // G.DrawPolygon(new Pen(Color.White, 4), Pts.ToArray()); // GraphicsInterface has DrawPolygon? No, DrawLines or FillPolygon
                    // GraphicsInterface.DrawPolygon is not standard? GraphicsInterface has DrawLines.
                    // ImageSharpGraphicsInterface: DrawLines(Pen, PointF[]). Close loop manually?
                    var penWhite4 = new Pen(Color.White, 4);
                    G.DrawLines(penWhite4, Pts.ToArray()); 
                    if(Pts.Count > 1) G.DrawLine(penWhite4, Pts.Last(), Pts.First()); // Close loop
                ...
                foreach (var a in OriginalCombinedSilk)
                {
                    ...
                    var penRed9 = new Pen(Color.Red, 9);
                    G.DrawLines(penRed9, Pts.ToArray());
                    if (Pts.Count > 1) G.DrawLine(penRed9, Pts.Last(), Pts.First());
                ...
                B.SaveAsPng(target + "_renderbase.png");
                ...
                }
                {
                    ...
                    for (int x = 0; x < iW; x++)
                    {
                        ...
                        for (int y = 0; y < iH; y++)
                        {
                            ...
                            Color C = Color.FromRgb((byte)(R), (byte)(R), (byte)(R));
                            DistanceFieldBlur[x + y * iW] = (DistanceFieldBlur[x + y * iW] - minblurval) / (maxblurval - minblurval);
                            B2[x, y] = C;
                        }
                    }
                    B2.SaveAsPng(target + "_renderDistance.png");
                }


                B.SaveAsPng(target + "_renderbase_afterdistance.png");
...

                {
                    ...
                    for (int y = 0; y < iH; y++)
                    {
                        ...
                        {
                            var C = B[x, y];
                            var C0 = B[(x + iW - 1) % iW, y];
                            var C1 = B[(x + 1) % iW, y];
                            var C2 = B[x, (y - 1 + iH) % iH];
                            var C3 = B[x, (y + 1) % iH];
                        ...
                        if (i % 100 == 0)
                        {
                            ...
                            for (int y = 0; y < iH; y++)
                            {
                                for (int x = 0; x < iW; x++)
                                {
                                    ...
                                    B2[x, y] = Color.FromRgb((byte)(c1), (byte)(c1), (byte)(c1));

                                }
                            }
                            B2.SaveAsPng(target + "_" + (img++).ToString() + ".png");
...
/*
            ...
            private static void ArtGenerator_Flower(ProgressLog log, string outline, string soldermask, string target, string silk)
            {
                ...
                }
                {
                    ...
                    for (int x = 0; x < iW; x++)
                    {
                        ...
                        for (int y = 0; y < iH; y++)
                        {
                            ...
                            byte R = (byte)((a - minblurval) * 255.0 / (maxblurval - minblurval));
                            Color C = Color.FromRgb((byte)(R / 2 + 127), (byte)(R / 2 + 127), (byte)(R / 2 + 127));
                            DistanceFieldBlur[x + y * iW] = (DistanceFieldBlur[x + y * iW] - minblurval) / (maxblurval - minblurval);
                            B[x, y] = C;
                        }
                    }
                    B.SaveAsPng(target + "_renderDistance.png");
                }
                ...
                GerberArtWriter GOW = new GerberArtWriter();
                {
                    ...
                    //  G.Clear(Color.Black);
                    var GG = new ImageSharpGraphicsInterface(B2);
                    GG.Clear(Color.Gray);

                    FlowerThing FT = new FlowerThing();
                    FT.Build(DistanceFieldBlur, AngleField, iW, iH, CombinedOutline);
                    //GG.DrawImage(B, 0, 0); // Need to check if DrawImage B is feasible directly or need another overload
                    // Assuming ImageSharpGraphicsInterface has DrawImage(Image<Rgba32> ...)
                    GG.DrawImage(B, 0, 0, iW, iH); 
                    ...
                    B2.SaveAsPng(target + "_FlowerPrint.png");
                }
            ...
            public class FlowerThing
            {
                public class Item
                {
                    public virtual void Draw(GraphicsInterface G)
                    {
                ...
                public class Leaf : Item
                {
                    ...
                    public override void Draw(GraphicsInterface G)
                    {
                ...
                public class Stem : Item
                {
                    ...
                    public override void Draw(GraphicsInterface G)
                    {
                        ...
                        Pen Pp = new Pen(C, (float)Width);
                        Pp.StartCap = LineCap.Round;
                        Pp.EndCap = LineCap.Round;
                        //Pp.DashCap = DashCap.Round; // Not supported in Pen struct yet
                        G.DrawLine(Pp, (float)Start.X, (float)Start.Y, (float)End.X, (float)End.Y);
                    }
                }
            ...
            private static void ArtGenerator_CheckerField(ProgressLog log, string outline, string soldermask, string target, string silk)
            {
                ...
                foreach (var a in CombinedOutline)
                {
                    ...
                    G.DrawLines(penWhite, Pts.ToArray());
                    if (Pts.Count > 1) G.DrawLine(penWhite, Pts.Last(), Pts.First());
                ...
                }
                {
                    ...
                    for (int x = 0; x < iW; x++)
                    {
                        ...
                        for (int y = 0; y < iH; y++)
                        {
                            var a = DistanceFieldBlur[x + y * iW];
                            //a = Math.Sqrt(a);
                            Color C = Color.FromRgb((byte)((a - minblurval) * 255.0 / (maxblurval - minblurval)), (byte)0, (byte)0);
                            B[x, y] = C;
                        }
                    }
                    B.SaveAsPng(target + "_renderDistance.png");
...

                {
                    ...
                    for (int x = 0; x < iW - 1; x++)
                    {
                        ...
                        for (int y = 0; y < iH - 1; y++)
                        {
                            ...
                            Color C = Color.FromRgb((byte)(255 - Math.Min(255, (int)(AngleDeg * (255.0 / 360.0)))), (byte)0, (byte)0);
                            if (B3[x, y].R == 255)
                            {
                                Color C2 = Color.FromRgb((byte)(vv), (byte)(vv), (byte)(vv));
                                B2[x, y] = C2;
                            }
                            else
                            {
                    ...
                    B.SaveAsPng(target + "_renderAngle.png");
                    B2.SaveAsPng(target + "_artwork.png");
                }
            ...
            public static void WriteBitmapToGerber(string target, ParsedGerber Outline, double ResX, Image<Rgba32> B2, int threshold = 0, bool fittooutline = true, bool cliptooutline = false)
            {
                int iW = B2.Width;
                int iH = B2.Height;
                double ResY = ResX;
            ...
            private static void ArtGenerator_FlowField(ProgressLog log, string outline, string soldermask, string target, string silk)
            {
                ...
                foreach (var a in CombinedOutline)
                {
                    ...
                    G.DrawLines(new Pen(Color.White, 1), Pts.ToArray());
        ...
        public class ArtSet
        {
            ...
            public List<Graphic> GraphicsInterface = new List<Graphic>();
    ...
    {
        public void WriteImageToBounds(Image<Rgba32> B, Bounds sizeofbitmap, Bounds rectangletobindto)
        {
            ...
            section = RectangleF.Intersect(section, contain);
            ...
            for (int y = 0; y < YPIXELS; y++)
            {
                ...
                {
                    Rgba32 C = B[x + ssx, B.Height - 1 - (y + ssy)];
                    if ((invert == false && C.R > threshold) || (invert == true && C.R < threshold))
                    {
                        if (active == false)
                        {
