...
namespace GerberLibrary.Core.Algorithms
{
    ...
    public class QuadTreeNode
    {
        ...
        {
            graphics.DrawRectangle(new GerberLibrary.Core.Primitives.Pen(contained > 0 ? Color.Red : Color.Yellow, (float)((xend-xstart)*0.001)), (float)xstart, (float)ystart, (float)xend - (float)xstart , (float)yend - (float)ystart);
        ...
        public void DrawArt(GraphicsInterface graphics, Color Col)
        {
            if (contained == 0)
            {
                graphics.FillRectangle(new GerberLibrary.Core.Primitives.SolidBrush(Col), (float)xstart, (float)ystart, (float)xend - (float)xstart - 1, (float)yend - (float)ystart - 1);
            }
            foreach (var C in Children)
            {
