...
namespace GerberLibrary.Core
{
    ...
    public class BoardContainer
    {
        ...
        public string Name = "";
        [XmlIgnore]
        public Rgba32 color = Color.LightGray;
        ...
        /*
        public void DrawLabel(GraphicsInterface graphics, Font font, Rectangle rectangle, bool darkedge = false, float fontangle = 0)
        {
            if (darkedge) graphics.FillRectangle(new SolidBrush(Color.FromRgb(30, 30, 30)), new Rectangle(rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height));
...
/*                var Pts = Root.Find("Board", false).First().Find("Points", false);
        ...
        */
        ...
        public static void RenderPackage(GraphicsInterface g, double x, double y, double angle, string packageName, BoardSide side)
        {
            ...
            switch(packageName.Trim())
            {
                ...
                case "C5B4.5":
                case "CAP-5.08/7.6x5":
                    RenderBigCap(g,7, 4.0f, Color.FromRgb(200, 60, 60));
                    
                    break;

                case "RES_2512":
                    RenderSMD2Pin(g, 25, 12, Color.FromRgb(60, 60, 60));                    
                    break;
                case "R0603":
                case "0603":
                    RenderSMD2Pin(g, 6, 3, Color.FromRgb(60, 60, 60));
                    break;
                case "L0805":
                    RenderSMD2Pin(g, 8, 5, Color.FromRgb(60, 70, 80));
                    break;
                case "0402":
                    RenderSMD2Pin(g, 4, 2, Color.FromRgb(160, 130, 60));
                    break;
                case "C0603":
                    RenderSMD2Pin(g, 6, 3, Color.FromRgb(160, 130, 60));
                    break;
                case "C3225":
                    RenderSMD2Pin(g, 32*10/25.4f, 25*10/25.4f, Color.FromRgb(160, 130, 60));
                    break;
                case "FERRITE_1812":
                    RenderSMD2Pin(g, 18, 12, Color.FromRgb(20, 20, 20));
                    break;
                case "RES_0805":
                case "R0805":
                case "0805":
                    RenderSMD2Pin(g, 8, 5, Color.FromRgb(60, 60, 60));
                    break;
                case "C0805":
                    RenderSMD2Pin(g, 8, 5, Color.FromRgb(160, 130, 60));
                    break;
                case "SO14":
                case "SOIC14":
                case "SOIC-14/150mil":
        ...
        private static void DrawMic(GraphicsInterface g)
        {
            float L = 2.95f;
            float W = 3.76f;

            g.FillRectangle(new SolidBrush(Color.FromRgb((byte)(200),(byte)(180),(byte)( 20))), -L / 2, -W / 2, L, W);

            g.FillEllipse(new SolidBrush(Color.Black), new RectangleF(-0.25f, W / 2 - 1.184f + 0.25f, 0.5f, 0.5f));

        }

        private static void RenderPinHeader(GraphicsInterface g, int wpin, int hpin, float pinspacing, bool shroud = false)
        {
            float L = (wpin ) * pinspacing;
            float W = (hpin ) * pinspacing; ;

            g.FillRectangle(new SolidBrush(Color.FromRgb(30, 30, 30)), -L / 2, -W / 2, L, W);
            ...
            for (int p = 0; p < wpin; p++)
            {
                for (int q = 0; q < hpin; q++)
                {
                    ...
                    g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 130)), x,y,pinw,pinw);
                }
            ...
            if (shroud)
            {
                g.DrawRectangle(new Pen(Color.FromRgb(40, 40, 40),1.4f), -L / 2, -W / 2, L, W);

            }
        }
        private static void RenderSot23_5(GraphicsInterface g)
        {
            float L = 3;
            float W = 1.4f;

            g.FillRectangle(new SolidBrush(Color.FromRgb(60, 60, 60)), -L / 2, -W / 2, L, W);
            ...
            for (int p = 0; p < sidepins; p++)
            {
                ...
                pinrect.Y = -1.25f;
                if (top[p]) g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 160)), pinrect);

                pinrect.Y = 1.25f - 0.5f;
                if (bottom[p]) g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 160)), pinrect);
            }

            pinrect.X = (0 - offs) * pinspacing - pinrect.Width/2;
            pinrect.Y = -0.4f;

            g.FillEllipse(new SolidBrush(Color.FromRgb(190, 190, 190)), pinrect);
        }

        private static void RenderBigTrim(GraphicsInterface g)
        {
            float W = 2.54f*4;
            float H = 2.54f*2;
            g.FillRectangle(new SolidBrush(Color.FromRgb(20, 20, 160)), -W / 2, -H / 2, W, H);
            float L = 2.54f;
            g.FillEllipse(new SolidBrush(Color.FromRgb(145, 145, 10)), -W / 2 , H / 2 - L, L, L);

            g.FillRectangle(new SolidBrush(Color.FromRgb(120, 120, 0)), -W / 2, H /2 - L + L/2 -0.1f, L, 0.2f);
        }

        private static void RenderToggleSwitch(GraphicsInterface g)
        {
            float W = 23;
            float H = 11;
            g.FillRectangle(new SolidBrush(Color.FromRgb(130, 130, 130)), -W / 2, -H / 2, W, H);
            float L = 5.0f;
            g.FillEllipse(new SolidBrush(Color.FromRgb(145, 145, 145)), -L / 2, -L / 2, L, L);
        }

        private static void RenderJack(GraphicsInterface g)
        {
            float W = 10;
            float H = 10;
            g.FillRectangle(new SolidBrush(Color.FromRgb(30, 30, 30)), -W / 2, -H / 2, W, H);
            float L = 9.5f;
            g.FillEllipse(new SolidBrush(Color.FromRgb(35, 35, 35)), -L / 2, -L / 2, L, L);
            L = 3.5f;
            g.FillEllipse(new SolidBrush(Color.FromRgb(2, 2, 2)), -L / 2, -L / 2, L, L);
        }

        private static void RenderButton(GraphicsInterface g)
        {
            float W = 6;
            float H = 6;
            g.FillRectangle(new SolidBrush(Color.FromRgb(120, 120, 120)), -W / 2, -H / 2, W, H);
            float L = 6;
            g.FillEllipse(new SolidBrush(Color.FromRgb(200, 180, 0)), -L / 2, -L / 2, L, L);

        }

        private static void RenderPot(GraphicsInterface g)
        {
            float W = 9;
            float H = 9;
            g.FillRectangle(new SolidBrush(Color.FromRgb(120, 120, 120)), -W / 2, -H / 2, W, H);
        ...
        private static void RenderQFN(GraphicsInterface g, float W, float H, int wpins, int hpins, float spacingbetweenpins)
        {

            g.FillRectangle(new SolidBrush(Color.FromRgb(50, 50, 50)), -W / 2, -H / 2, W, H);
            ...
            SolidBrush b = new SolidBrush(Color.FromRgb(160, 160, 160));
        ...
        float dotsize = 0.5f;
            ...
            g.FillEllipse(new SolidBrush(Color.FromRgb(190, 190, 190)), dot);


        }
        public static void RenderECap(GraphicsInterface g, float candiameter)
        {
            ...
            g.FillPolygon(new SolidBrush(Color.FromRgb((byte)(76),(byte)(80),(byte)(85))), polyvert.ToArray());
            g.FillEllipse(new SolidBrush(Color.FromRgb(110, 110, 110)), -L / 2, -L / 2, L, L);

        }
        private static void RenderDO214Diode(GraphicsInterface g)
        {
            RenderSMD2Pin(g, 3.2f * 100 / 25.4f, 1.6f * 100 / 25.4f, Color.FromRgb(60, 60, 60), Color.FromRgb(180, 180, 180), Color.FromRgb(60, 60, 60));
        }

        private static void RenderLed(GraphicsInterface g)
        {
            float L = 3;
            g.FillEllipse(new SolidBrush(Color.FromRgb(200, 180, 0)), -L / 2, -L / 2, L, L);
        }

        private static void RenderTrimPot(GraphicsInterface g)
        {
            float L = 3.0f;
            float W = 3.8f;
            float extendH = 4.1f / 2;
            g.FillRectangle(new SolidBrush(Color.FromRgb(120, 120, 120)), -L / 2, -W / 2, L, W);

            g.FillEllipse(new SolidBrush(Color.FromRgb(110, 110, 110)), -L / 2, -L / 2, L, L);

            g.FillRectangle(new SolidBrush(Color.FromRgb(90, 90, 90)), -L*0.6f / 2, -L * 0.1f / 2, L * 0.6f, L * 0.1f);
            g.FillRectangle(new SolidBrush(Color.FromRgb(90, 90, 90)), -L * 0.1f / 2, -L * 0.6f / 2, L * 0.1f, L * 0.6f);
            ...
            for (int p = 0; p < sidepins; p++)
            {
                ...
                if (bottom[p]) g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 160)), pinrectpad);
            }
        }

        private static void RenderSOT363(GraphicsInterface g)
        {
            float L = 2;
            float W = 1.25f;
        ...
        private static void RenderSOT223(GraphicsInterface g)
        {
            float L = 6.5f;
            float W = 3.5f;
        ...
        private static void RenderSot23(GraphicsInterface g)
        {
            float L = 3;
            float W = 1.4f;
        ...
        private static void RenderTantalum(GraphicsInterface g)
        {
            RenderSMD2Pin(g, 3.2f * 100 / 25.4f, 1.6f* 100 / 25.4f, Color.FromRgb((byte)(160),(byte)(100),(byte)(30)), Color.FromRgb((byte)(160),(byte)(10),(byte)(10)));
        }

        private static void RenderSot323(GraphicsInterface g)
        {
            float L = 2;
            float W = 1.25f;

            g.FillRectangle(new SolidBrush(Color.FromRgb((byte)(60),(byte)(60),(byte)(60))), -L / 2, -W / 2, L, W);
        ...
        private static void RenderBigCap(GraphicsInterface g, float L, float W, Color color)
        {
            g.FillRectangle(new SolidBrush(color), -L/ 2 , -W / 2, L,W);

        }

        public static void RenderSMD2Pin(GraphicsInterface g, float w, float h, Color BodyColor, Color ?pin1mark = null, Color? pin2mark = null)
        {
            w *=  25.4f/100.0f;
            h *=  25.4f/100.0f;

            g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 160)), -w / 2, -h / 2, w ,h);
        ...
        public static void RenderSOIC(GraphicsInterface g, int v)
        {

            int sidepins = v / 2;
            float length = (sidepins - 1) * 1.27f;
            g.FillRectangle(new SolidBrush(Color.FromRgb(60, 60, 60)), -length / 2 - 0.5f, -3.9f / 2, length + 1, 3.9f);
            ...
            for(int p = 0;p<sidepins ;p++)
            {
                ...
                pinrect.Y = -3;
                g.FillRectangle(new SolidBrush(Color.FromRgb(160, 160, 160)), pinrect);
        ...
        public static void RenderSSOP(GraphicsInterface g, int v, float W = 3.9f)
        {
            ...
            g.FillRectangle(new SolidBrush(Color.FromRgb(60, 60, 60)), -length / 2 - 0.5f, -W / 2, length + 1, W);
            RectangleF pinrect = new RectangleF(0, 0, 0.4f, 1);
            float offs = (sidepins / 2);
            if ((sidepins % 2) != 1) offs -= 0.5f;
