    ...
    public class SickOfBeige : GerberSet
    {
        public string MinimalDXFSave(string outputfile, double offset = 3.0, double holediameter = 3.2)
        {
            ...
            Console.WriteLine("Report: {0} holes created in case ({1} spacers and {1} screws needed!)", Holes.Count, Holes.Count * 2);
            {
                ...
                int w = (int)((BB.Width() ) * scalefac) + 6;
                int h = (int)((BB.Height()) * scalefac) + 6;

                using(Image<Rgba32> B = new Image<Rgba32>(w, h))
                {
                    B.Mutate(G => {
                        G.Fill(Color.White);
                        
                        Matrix3x2 matrix = Matrix3x2.CreateTranslation((float)-BB.TopLeft.X, (float)-BB.TopLeft.Y) *
                                           Matrix3x2.CreateScale(scalefac) *
                                           Matrix3x2.CreateTranslation(3, 3);
                        
                        DrawingOptions options = new DrawingOptions { Transform = matrix };

                        var isPen1 = new SolidPen(Color.Black, 0.1f);
                        var isPen2 = new SolidPen(Color.FromRgba(160, 160, 160, 255), 0.1f);
                        
                        // GerberImageCreator.ApplyAASettings(G); // ImageSharp defaults to high quality

                        foreach (var a in Holes)
                        {
                             // EllipsePolygon(x, y, w, h) -> Centered at x,y with width/height w,h
                             G.Draw(options, isPen1, new EllipsePolygon((float)a.X, (float)a.Y, (float)holediameter, (float)holediameter));
                        }
...

                        {
                            ...
                            for (int i = 0; i < Pl.Vertices.Count; i++)
                            {
                             ...
                             Points.Add(Pl.Vertices[0].ToF());
                              var path2 = new PathBuilder().AddLines(Points).Build(); G.Draw(options, isPen2, path2);
                        }
                    });

                    var ImagePNG = outputfile + ".png";
                    B.SaveAsPng(ImagePNG);
                    return ImagePNG;
                }
            }
        }        
