...
using StringAlignment = TilingLibrary.Compatibility.Primitives.StringAlignment;
...
namespace TINRSART.SVG
{
    ...
    public class Polygon
    {
        ...
        public void Draw(Graphics G, float linewidth = 1.0f)
        {
            ...
            if (Filled)
            {
                G.FillPolygon(new SolidBrush(Color.FromRgba(r, g, b, a)), Points.ToArray());
            }
            else
            {
                if (Closed)
                {
                    G.DrawPolygon(new Pen(Color.FromRgba(r, g, b, a), linewidth), Points.ToArray());
                }
                else
                {
                    G.DrawLines(new Pen(Color.FromRgba(r, g, b, a), linewidth), Points.ToArray());
                }
            }
        }
    ...
    public class SVGWriter
    {
        ...
        {
            var bgC = BG.ToPixel<Rgba32>();
            string bgcolorstring = String.Format("#{0:X2}{1:X2}{2:X2}", bgC.R, bgC.G, bgC.B);
    ...
    public static class SVGThings
    {
        ...
        abstract class SVGStyledNode : SVGNode
        {
            ...
            public static Color ParseColor(string color)
            {
                ...
                if (color.StartsWith("rgb"))
                {
                    ...
                    var S = color.Split(',');
                    Color R = Color.FromRgba((byte)int.Parse(S[0]), (byte)int.Parse(S[1]), (byte)int.Parse(S[2]), 255);
                    return R;
                }
        ...
        class SVGRect : SVGStyledNode
        {
            ...
            public override List<List<Polygon>> GetPolygons()
            {
                ...
                {
                    var fc = FillColor.ToPixel<Rgba32>();
                    var sc = StrokeColor.ToPixel<Rgba32>();
                    p.r = fc.R;
                    p.g = fc.G;
                    p.b = fc.B;
                    p.a = fc.A;
