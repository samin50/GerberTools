...
using GraphicsInterface = TilingLibrary.Compatibility.GraphicsInterface;
...
namespace Artwork
{
    public class DelaunayBuilder
    {
        ...
        public void Build(QuadTreeNode Mask, double compensateangle  = 0.0)
        {
          //  M = new Mesh();
            ...
            foreach (var a in M.Triangles)
            {
                ...
                {
                    var D1 = Difference(new PointF((float)V1.X, (float)V1.Y), new PointF((float)V2.X, (float)V2.Y));
                    var D2 = Difference(new PointF((float)V2.X, (float)V2.Y), new PointF((float)V3.X, (float)V3.Y));
                    var D3 = Difference(new PointF((float)V3.X, (float)V3.Y), new PointF((float)V1.X, (float)V1.Y));
                    var D1L = Length(D1);
                    var D2L = Length(D2);
                    var D3L = Length(D3);
                    if (D1L < D2L && D1L<D3L)
        ...
        private PointF Difference(PointF p1, PointF p2)
        {
